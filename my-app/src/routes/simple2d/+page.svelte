	...
	onMount(() => {
		...
		class GameMap {
			...
			constructor(X, Y, MaxX, MaxY, cellWidth, cellHeight, defaultColor, mapMargin, rows, cols) {
				this.mapMarginBottom =
					this.mapMarginRight =
					this.mapMarginLeft =
					this.mapMarginTop =
						mapMargin ? mapMargin : 10;
				...
				console.log(
					`Map Generated: columns (${this.cellsByCol.length}) | rows (${this.cellsByCol[0].length})`
				);
				console.log(this);
			}
			...
			getCellsNear(x, y, range) {
				...
				this.cellsByCol.slice(approxMinCol, approxMaxCol).forEach((col) => {
					candidates.push(
						...col.slice(approxMinRow, approxMaxRow).filter((nonEmpty) => {
							return nonEmpty;
						})
					);
				});
		...
		function checkCollision(x, y, width, height) {
			...
			var colliders = colliders.sort((a, b) => {
				return (
					xOverlap(b.X, b.width, x, height) * yOverlap(b.Y, b.height, y, height) -
					xOverlap(a.X, a.width, x, height) * yOverlap(a.Y, a.height, y, height)
				);
			});
			var breachXr;
			var breachXl;
		...
		async function startGameLoop() {
            ...
            console.log(arrow); 
			let box = document.getElementById(arrow.id);
			let front = document.getElementById(arrow.frontID);
